...
export async function deactivateCustomer(id: string, reason: string, deactivatedBy: string) {
  const supabase = await createClient();

  if (!reason.trim()) return { error: 'A reason is required before deactivating an account.' };

  // Never deletes the row. Sets status to Inactive and records who/why/when,
  // so the full record (KYC data included) stays retrievable for
  // regulatory retention (min. 5 years under Nigerian AML/CBN rules).
  const { error } = await supabase
    .from('customers')
    .update({
      status: 'Inactive',
