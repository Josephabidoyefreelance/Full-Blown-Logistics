...
export default async function CustomersPage() {
  ...
  const [{ data: customers, error }, { data: staff }] = await Promise.all([
    supabase
      .from('customers')
      .select('id, name, type, contact, credit_limit, balance, status, client_since, assigned_to, currency, tax_rate, deactivation_reason, deactivated_at, deactivated_by')
    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
        <div className="min-w-[900px]">
          <div className="grid grid-cols-[repeat(6,minmax(0,1fr))_160px_140px] items-center gap-x-6 bg-neutral-50 px-4 py-2.5 text-[11px] uppercase text-neutral-500 dark:bg-neutral-800/60 dark:text-neutral-400">
            <div>Account</div>
            <div>Type</div>
            <div>Contact</div>
          ...
          {(customers ?? []).map((c) => (
            <div
              key={c.id}
              className="grid grid-cols-[repeat(6,minmax(0,1fr))_160px_140px] items-center gap-x-6 border-t border-neutral-100 px-4 py-2.5 text-sm dark:border-neutral-800"
            >
